...
无需任何命令行知识，双击 `novel-reader.exe` 或直接运行：
...
```bash
# 基本搜索
python -m novel_reader search "Python Programming"

# 按格式过滤
python -m novel_reader search "Python" --format epub

# 按语言过滤
python -m novel_reader search "三体" --language chinese
```
...
```bash
python -m novel_reader download <Z-Library-URL>
```
...
```bash
# 从书架中选择
python -m novel_reader read

# 直接打开指定书籍（系统默认阅读器）
python -m novel_reader read "Python Cookbook"

# EPUB 终端彩色阅读模式
python -m novel_reader read --terminal "Python Cookbook"
```

### 5. 查看书架

```bash
python -m novel_reader list
```
...
构建完成后，`dist/` 目录下会生成 `novel-reader.exe`（约 15 MB），可直接双击运行或在终端中使用：
...
rem CLI 模式
novel-reader.exe setup
novel-reader.exe search "Python"
novel-reader.exe list
```
...
├── build_exe.py                  # 一键构建脚本
├── novel_reader.spec             # PyInstaller 规格文件
├── books/                        # 下载的电子书存放目录
...
- 如认证失效，重新运行 `python -m novel_reader setup` 更新令牌
